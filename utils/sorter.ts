 ...
 */
export function sortRestaurantsByKeyword(restaurants: any[], keyword: string) {
  ...
  const scoredRestaurants = restaurants.map(
    (restaurant: { name: string; distance: string; rating: number }) => {
  ...
  return scoredRestaurants
    ...
    .map((r: { [x: string]: any; score: any }) => {
      // Remove the score field before returning (optional)
      const { score, ...restaurantWithoutScore } = r; // eslint-disable-line @typescript-eslint/no-unused-vars
